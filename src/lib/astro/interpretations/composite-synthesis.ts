...
import { describeAspect, describePlanetInHouse, frArticle, type Locale } from "./compose";
...
function buildCompositeLifeDomains(composite: CompositeChart, locale: Locale): CompositeLifeDomainReading[] {
  ...
  for (let houseNumber = 1; houseNumber <= 12; houseNumber++) {
    ...
    if (locale === "en") {
      const rulerPlace = rulerPoint
        ? ` In this relationship's composite chart, ${rulerName} sits in ${signMap[signOf(rulerPoint.longitude)].name}${
            rulerPoint.house ? `, house ${rulerPoint.house}` : ""
          } — that placement is where this domain's real story actually plays out for the bond.`
        : "";
      ...
      const rulerPlace = rulerPoint
        ? ` Dans le thème composite de cette relation, ${frArticle(rulerKey, rulerName)}${rulerName} se trouve en ${signMap[signOf(rulerPoint.longitude)].name}${
            rulerPoint.house ? `, maison ${rulerPoint.house}` : ""
          } — c'est là que se joue concrètement l'histoire réelle de ce domaine pour ce lien.`
        : "";
...
export function composeCompositeSynthesis(
  ...
  const lifeDomains = buildCompositeLifeDomains(composite, locale);
