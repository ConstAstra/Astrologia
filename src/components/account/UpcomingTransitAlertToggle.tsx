...
const TEXT = {
  fr: {
    title: "Un transit fort approche",
    subtitle: "Une notification 3 jours avant qu'un aspect majeur touchant une planète personnelle ne devienne exact.",
  ...
  en: {
    title: "A strong transit is coming",
    subtitle: "A notification 3 days before a major aspect touching a personal planet becomes exact.",
...
export function UpcomingTransitAlertToggle({
  initialOptIn,
  locale = "fr",
}: {
  ...
  async function enable() {
    ...
    try {
      ...
      const res = await fetch("/api/notifications/push-subscribe", {
        ...
        body: JSON.stringify({ endpoint: json.endpoint, keys: json.keys, feature: "upcomingTransitAlert" }),
      });
    ...
    try {
      ...
      if (subscription) {
        const res = await fetch("/api/notifications/push-subscribe", {
          method: "DELETE",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ endpoint: subscription.endpoint, feature: "upcomingTransitAlert" }),
        });
        // Un seul abonnement Push par appareil, partagé avec les trois autres
