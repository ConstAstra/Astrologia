...
> = {
  fr: {
    ...
    theirNamePlaceholder: "Iel",
    generate: "Générer ma carte",
    generating: "…",
    error: "Choisis un prénom et un signe pour chacun.",
    post: "⤓ Carte",
    story: "⤓ Story",
    sharing: "…",
  ...
  en: {
    ...
    theirNamePlaceholder: "Them",
    generate: "Generate my card",
    generating: "…",
    error: "Pick a name and a sign for each of you.",
    post: "⤓ Card",
    story: "⤓ Story",
    sharing: "…",
...
export function DuoTeaserForm({ locale = "fr" }: { locale?: Locale }) {
  const t = TEXT[locale];
  ...
  function buildUrl(format: Format) {
    const params = new URLSearchParams({
      nameA: nameA.trim() || (locale === "en" ? "You" : "Toi"),
      signA: signA ?? "",
      nameB: nameB.trim() || (locale === "en" ? "Them" : "Iel"),
      signB: signB ?? "",
      format,
      locale,
    });
    return `/api/share/duo?${params.toString()}`;
  }
    ...
    <div className="mx-auto max-w-xl">
      <div className="grid gap-6 sm:grid-cols-2">
        <div className="space-y-3">
          <div>
            <label className="mb-1 block text-sm text-muted" htmlFor="duo-nameA">
              {t.yourName}
            </label>
            <input
              id="duo-nameA"
          ...
          <SignPicker label={t.yourSign} value={signA} onChange={setSignA} locale={locale} />
        </div>
        <div className="space-y-3">
          <div>
            <label className="mb-1 block text-sm text-muted" htmlFor="duo-nameB">
              {t.theirName}
            </label>
            <input
              id="duo-nameB"
          ...
          <SignPicker label={t.theirSign} value={signB} onChange={setSignB} locale={locale} />
        </div>
      </div>
