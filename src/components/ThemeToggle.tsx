...
function toggle() {
  ...
  if (next === "night") html.dataset.theme = "night";
...
export function ThemeToggle({ className = "" }: { className?: string }) {
  return (
    ...
    >
      {/* Lune : visible en mode jour (Bloom), clic → passe en nuit. */}
      <svg viewBox="0 0 24 24" fill="currentColor" className="theme-icon-night h-3.5 w-3.5" aria-hidden="true">
        <path d="M20.4 14.7A8.5 8.5 0 0 1 9.3 3.6a.7.7 0 0 0-.9-.9 9.9 9.9 0 1 0 12.9 12.9.7.7 0 0 0-.9-.9Z" />
      </svg>
      {/* Soleil : visible en mode nuit, clic → passe en jour. */}
      <svg
        viewBox="0 0 24 24"
        fill="none"
        stroke="currentColor"
        strokeWidth={1.6}
        className="theme-icon-day h-3.5 w-3.5"
        aria-hidden="true"
      >
