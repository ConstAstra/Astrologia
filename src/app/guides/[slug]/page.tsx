...
import { GUIDE_DIAGRAMS } from "@/components/guides/GuideDiagrams";
...
export default async function GuideArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const dateLabel = new Date(guide.publishedAt).toLocaleDateString("fr-FR", { day: "numeric", month: "long", year: "numeric" });
  const Icon = GUIDE_ICONS[guide.slug];
  const Diagram = GUIDE_DIAGRAMS[guide.slug];
    ...
    <>
      ...
      <main className="flex-1">
        <article className="mx-auto max-w-2xl px-6 pb-16 pt-16">
          <Link href="/guides" className="text-xs text-muted hover:text-foreground">
            ← Tous les guides
          </Link>
          ...
          <p className="mt-3 text-xs text-muted">
            {dateLabel} · {guide.readingMinutes} min de lecture
          </p>
          ...
          <div className="mt-10 border-t border-border-soft pt-6">
            <p className="text-xs text-muted">Guide suivant</p>
            <Link href={`/guides/${next.slug}`} className="font-display mt-1 block text-lg hover:text-gold-strong">
              {next.title} →
            </Link>
          </div>
        </article>
