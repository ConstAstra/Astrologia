...
export default async function CompositePage({
  ...
  const compositeThemeLabel =
    locale === "en"
      ? `composite chart of a ${relationshipMeta[relationshipType].label.toLowerCase()}`
      : `thème composite (${relationshipMeta[relationshipType].label.toLowerCase()})`;
  const compositeFacts = buildChartFacts(composite, locale);
  const grimoireDomains =
    (await getOrGenerateDeepSynthesis(
      { type: "composite", profileId: primaryProfileId, secondaryProfileId, relationshipType, locale },
      compositeFacts,
